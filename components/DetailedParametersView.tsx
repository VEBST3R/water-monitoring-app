...
const MiniChart: React.FC<{ 
  ...
  if (range < 0.01) {
    ...
    // Центруємо лінію посередині діаграми коли значення однакові
    let y;
    if (maxValue === minValue) {
      y = height / 2; // Центруємо горизонтальну лінію
    } else {
      // Нормалізуємо значення з невеликим відступом від країв
      const normalizedY = ((value - minValue) / range);
      y = height - (normalizedY * (height * 0.8) + height * 0.1); // 10% відступ зверху і знизу
    }
    return `${x},${y}`;
  }).join(' ');    return (
...
const getParameterDisplayConfig = (paramKey: string, value?: number): { 
  ...
  switch (paramKey) {
    case 'pH':
      ...
      if (value < 6.5) color = '#FF9800'; // Orange (Acidic)
      else if (value <= 8.5) color = '#4CAF50'; // Green (Optimal)
      else color = '#9C27B0'; // Purple (Alkaline)
      break;
    case 'temperature':
      unit = '°C';
      icon = 'thermometer-outline';
      optimalRange = '15-25°C';
      if (value < 10) color = '#2196F3'; // Blue (Cold)
      else if (value <= 25) color = '#4CAF50'; // Green (Optimal)
      else color = '#FF5722'; // Red (Hot)
      break;
    case 'tds':
      unit = 'ppm';
      icon = 'water-outline';
      optimalRange = '<300 ppm';
      if (value < 150) color = '#4CAF50'; // Green (Excellent)
      else if (value <= 300) color = '#FF9800'; // Orange (Acceptable)
      else color = '#F44336'; // Red (Poor)
      break;
    case 'turbidity':
      unit = 'NTU';
      icon = 'eye-outline';
      optimalRange = '<1 NTU';
      if (value < 1) color = '#4CAF50'; // Green (Clear)
      else if (value <= 5) color = '#FF9800'; // Orange (Slightly Turbid)
      else color = '#795548'; // Brown (Turbid)
      break;
    default:
...
const getWaterQualityAssessment = (parameters: WaterParameters) => {
  ...
  if (parameters.pH !== undefined) {
    if (parameters.pH < 6.0) {
      ...
      if (overallStatus === 'excellent') overallStatus = 'acceptable';
  ...
  if (parameters.temperature !== undefined) {
    if (parameters.temperature < 5) {
      ...
      if (overallStatus !== 'critical') overallStatus = 'poor';
  ...
  if (parameters.turbidity !== undefined) {
    if (parameters.turbidity > 10) {
      ...
      if (overallStatus !== 'critical' && overallStatus !== 'poor') overallStatus = 'acceptable';
    ...
    <View style={styles.outerContainer}>
      ...
      >
        <ThemedText type="title" style={styles.title}>Параметри якості води</ThemedText>
          ...
          const { color, displayValue, unit, icon, optimalRange } = getParameterDisplayConfig(config.key, numericValue);
            // Використовуємо збережені історичні дані з сервера
          const chartData = historicalData[config.key] || [];
            return (
            <TouchableOpacity 
              key={config.key} 
              style={styles.parameterCard}
              onPress={() => chartData.length > 0 && openExpandedChart(config, { color, unit, optimalRange, icon }, chartData)}
              activeOpacity={chartData.length > 0 ? 0.7 : 1}
            >
